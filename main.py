...
async def scan(ctx, *, url):
  if Is_VTToken_found == "Yes":
    ...
    log_handler(space_T="b", pointer_T="d", message=f'Data: "{url}"')
    await ctx.send(embed=guilded.Embed(title="Scan Command (Tip)", description=f"This command is not yet configured. Error:VTAPI-Not-Found", color=embed_color))
...
try:
  ...
  if virustotal_token == "":
    ...
    if os.environ['VTToken']:
      log_handler(space_T="b", pointer_T="c", message="VTToken found!")
      VirToken = os.environ['VTToken']
      Is_VTToken_found = "Yes"
    else:
      log_handler(space_T="b", pointer_T="c", message="VTToken Not Found! Disabling link scan feature.")
      Is_VTToken_found = "No"
  else:
  ...
  if guilded_token == "":
    ...
    if os.environ['GToken']:
      log_handler(space_T="b", pointer_T="c", message="GToken found!")
